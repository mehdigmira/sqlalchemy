...
class SimpleResultMetaData(ResultMetaData):
    ...
    def _contains(self, value: Any, row: Row[Unpack[Tuple[Any, ...]]]) -> bool:
...
) -> Callable[[Iterable[Any]], Row[Unpack[Tuple[Any, ...]]]]:
...
class Result(_WithKeys, ResultInternal[Row[Unpack[_TP]]]):
    ...
    def __iter__(self) -> Iterator[Row[Unpack[_TP]]]:
        raise NotImplementedError()

        return self._iter_impl()

    def __next__(self) -> Row[Unpack[_TP]]:
        return self._next_impl()

    def partitions(
        self, size: Optional[int] = None
    ) -> Iterator[Sequence[Row[Unpack[_TP]]]]:
    ...
    def fetchall(self) -> Sequence[Row[Unpack[_TP]]]:
        """A synonym for the :meth:`_engine.Result.all` method."""

        return self._allrows()

    def fetchone(self) -> Optional[Row[Unpack[_TP]]]:
        """Fetch one row.
    ...
    def all(self) -> Sequence[Row[Unpack[_TP]]]:
    ...
    def first(self) -> Optional[Row[Unpack[_TP]]]:
    ...
    def one_or_none(self) -> Optional[Row[Unpack[_TP]]]:
    ...
    def freeze(self) -> FrozenResult[Unpack[_TP]]:
    ...
    def merge(self, *others: Result[Any]) -> MergedResult[Unpack[_TP]]:
...
class FrozenResult(Generic[Unpack[_TP]]):
    ...
    def __init__(self, result: Result[Unpack[_TP]]):
    ...
    def with_new_rows(
        self, tuple_data: Sequence[Row[Unpack[_TP]]]
    ) -> FrozenResult[Unpack[_TP]]:
    ...
    def __call__(self) -> Result[Unpack[_TP]]:
        result: IteratorResult[Unpack[_TP]] = IteratorResult(
...
class IteratorResult(Result[Unpack[_TP]]):
...
class ChunkedIteratorResult(IteratorResult[Unpack[_TP]]):
...
class MergedResult(IteratorResult[Unpack[_TP]]):
    ...
    def __init__(
        self,
        cursor_metadata: ResultMetaData,
        results: Sequence[Result[Unpack[_TP]]],
    ):
